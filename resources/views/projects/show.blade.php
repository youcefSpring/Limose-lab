<x-app-layout>
    ...
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 lg:mb-8">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-semibold">{{ $project->title ?? __('projects.project_details') }}</h1>
                <p class="text-zinc-500 dark:text-zinc-400 text-sm mt-1">{{ __('projects.project') }} #{{ $project->id ?? '---' }}</p>
            </div>
        </div>
        <span class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium
            ...
            {{ __('projects.' . ($project->status ?? 'active')) }}
        </span>
    </header>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="glass-card rounded-2xl p-5 lg:p-6">
                <h2 class="text-lg font-semibold mb-5">{{ __('projects.project_overview') }}</h2>
                <div class="space-y-5">
                    <div>
                        <h3 class="text-sm font-medium text-zinc-500 dark:text-zinc-400 mb-2">{{ __('projects.description') }}</h3>
                        <p class="text-zinc-900 dark:text-zinc-100 whitespace-pre-line leading-relaxed">{{ $project->description }}</p>
                    </div>

                    @if($project->objectives)
                        <div>
                            <h3 class="text-sm font-medium text-zinc-500 dark:text-zinc-400 mb-2">{{ __('projects.objectives') }}</h3>
                    ...
                    @if($project->methodology)
                        <div>
                            <h3 class="text-sm font-medium text-zinc-500 dark:text-zinc-400 mb-2">{{ __('projects.methodology') }}</h3>
            ...
            <!-- Progress Tracking -->
            <div class="glass-card rounded-2xl p-5 lg:p-6">
                <h2 class="text-lg font-semibold mb-5">{{ __('projects.project_progress') }}</h2>
                <div class="space-y-4">
                    <div>
                        <div class="flex justify-between mb-3">
                            <span class="text-sm font-medium">{{ __('projects.overall_progress') }}</span>
                    ...
                    @if($project->milestones && $project->milestones->count() > 0)
                        <div class="pt-5 border-t border-black/5 dark:border-white/5">
                            <h4 class="text-sm font-semibold mb-4">{{ __('projects.milestones') }}</h4>
                            <div class="space-y-3">
                                    ...
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-1 flex items-center gap-1">
                                                ...
                                                {{ __('projects.due') }}: {{ $milestone->due_date?->format('M d, Y') }}
                                            </p>
                                        </div>
                                    </div>
                                @endforeach
            ...
            <div class="glass-card rounded-2xl p-5 lg:p-6">
                <div class="flex justify-between items-center mb-5">
                    <h2 class="text-lg font-semibold">{{ __('projects.team_members') }} ({{ $project->members?->count() ?? 0 }})</h2>
                ...
                @else
                    <p class="text-center text-zinc-500 dark:text-zinc-400 py-8">
                        {{ __('projects.no_team_members_assigned') }}
                    </p>
                @endif
            </div>
            ...
            <div class="glass-card rounded-2xl p-5 lg:p-6">
                <div class="flex justify-between items-center mb-5">
                    <h2 class="text-lg font-semibold">{{ __('projects.experiments') }} ({{ $project->experiments?->count() ?? 0 }})</h2>
                                ...
                                <div class="flex justify-between items-start gap-3">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-1 flex items-center gap-2">
                                            ...
                                            <span>•</span>
                                            <span>{{ __('projects.by') }} {{ $experiment->researcher?->name }}</span>
                                        </p>
                                    </div>
                ...
                @else
                    <p class="text-center text-zinc-500 dark:text-zinc-400 py-8">
                        {{ __('projects.no_experiments_recorded') }}
                    </p>
                @endif
            </div>
        </div>
        ...
        <div class="space-y-6">
            <!-- Project Information -->
            <div class="glass-card rounded-2xl p-5 lg:p-6">
                <h2 class="text-lg font-semibold mb-5">{{ __('projects.project_information') }}</h2>
                <dl class="space-y-4">
                    <div>
                        <dt class="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1">{{ __('projects.principal_investigator') }}</dt>
                        <dd class="text-sm font-medium">{{ $project->principal_investigator?->name }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1">{{ __('projects.start_date') }}</dt>
                        <dd class="text-sm font-medium font-mono">{{ $project->start_date?->format('M d, Y') }}</dd>
                    </div>
                    @if($project->end_date)
                        <div>
                            <dt class="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1">{{ __('projects.end_date') }}</dt>
                            <dd class="text-sm font-medium font-mono">{{ $project->end_date->format('M d, Y') }}</dd>
                        </div>
                    @endif
                    <div>
                        <dt class="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1">{{ __('projects.duration') }}</dt>
                        <dd class="text-sm font-medium">
                            <span class="font-mono">{{ $project->start_date?->diffInDays($project->end_date ?? now()) }}</span> {{ __('projects.days') }}
                        </dd>
                    </div>
                    @if($project->budget)
                        <div>
<dt class="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1">{{ __('projects.budget') }}</dt>
                        <dd class="text-sm font-medium font-mono">{{ number_format($project->budget, 2) }} {{ __('projects.usd') }}</dd>
                        </div>
                    @endif
                    @if($project->funding_source)
                        <div>
                            <dt class="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1">{{ __('projects.funding_source') }}</dt>
                            <dd class="text-sm font-medium">{{ $project->funding_source }}</dd>
                        </div>
                    @endif
                    <div>
                        <dt class="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1">{{ __('projects.created_at') }}</dt>
            ...
            <!-- Quick Stats -->
            <div class="glass-card rounded-2xl p-5 lg:p-6">
                <h2 class="text-lg font-semibold mb-5">{{ __('projects.quick_stats') }}</h2>
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-zinc-600 dark:text-zinc-400">{{ __('projects.team_members') }}</span>
                        <span class="text-xl font-bold font-mono text-accent-rose">{{ $project->members?->count() ?? 0 }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-zinc-600 dark:text-zinc-400">{{ __('projects.experiments') }}</span>
                        <span class="text-xl font-bold font-mono text-accent-emerald">{{ $project->experiments?->count() ?? 0 }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-zinc-600 dark:text-zinc-400">{{ __('projects.publications') }}</span>
            ...
            @can('update', $project)
                <div class="glass-card rounded-2xl p-5 lg:p-6">
                    <h2 class="text-lg font-semibold mb-4">{{ __('projects.actions') }}</h2>
                    <div class="space-y-2">
