...
<head>
    ...
    <title>{{ $settings['site_name'] ?? config('app.name', 'RLMS') }} - {{ $settings['site_tagline'] ?? 'Système de Gestion de Laboratoire de Recherche' }}</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body { font-family: 'Source Sans 3', sans-serif; color: var(--cool-gray); overflow-x: hidden; }
        h1, h2, h3, h4, h5, h6 { font-family: 'Playfair Display', serif; color: var(--cardinal-red); }
        ...
        .logo { font-family: 'Playfair Display', serif; font-size: 1.75rem; font-weight: 700; color: var(--cardinal-red); text-decoration: none; }
        ...
        .hero h1 { font-size: 3.5rem; line-height: 1.3; margin-bottom: 1.5rem; animation: fadeInUp 1s ease; }
        ...
        .section-title { text-align: center; font-size: 2.75rem; margin-bottom: 1rem; }
        .section-subtitle { text-align: center; font-size: 1.15rem; color: var(--cool-gray); margin-bottom: 4rem; }
        ...
        .about-content p { font-size: 1.15rem; line-height: 1.8; margin-bottom: 1.5rem; color: var(--cool-gray); }
        ...
        .stat-number { font-size: 2.75rem; font-weight: 700; color: var(--cardinal-red); font-family: 'Playfair Display', serif; }
        ...
        .team-avatar { width: 130px; height: 130px; border-radius: 50%; background: linear-gradient(135deg, var(--cardinal-red), var(--cardinal-dark)); margin: 0 auto 1.5rem; display: flex; align-items: center; justify-content: center; color: white; font-size: 2.75rem; font-weight: 700; font-family: 'Playfair Display', serif; }
        ...
        .research-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 2rem; max-width: 1200px; margin: 0 auto; }
        ...
        .research-image { width: 100%; height: 180px; background: linear-gradient(135deg, var(--cardinal-red), var(--cardinal-dark)); display: flex; align-items: center; justify-content: center; font-size: 3.5rem; }
        .research-content { padding: 1.75rem; }
        .research-card h3 { font-size: 1.35rem; margin-bottom: 0.75rem; }
        .research-card p { line-height: 1.6; color: var(--cool-gray); margin-bottom: 0.75rem; font-size: 0.9rem; }
        .research-meta { font-size: 0.85rem; color: #999; }
        ...
        .event-header { background: linear-gradient(135deg, var(--cardinal-red), var(--cardinal-dark)); padding: 1.75rem; color: white; text-align: center; }
        .event-date { font-size: 2.75rem; font-weight: 700; font-family: 'Playfair Display', serif; line-height: 1; }
        .event-month { font-size: 1.1rem; text-transform: uppercase; letter-spacing: 2px; margin-top: 0.5rem; }
        .event-content { padding: 1.75rem; }
        .event-card h3 { font-size: 1.35rem; margin-bottom: 1rem; }
        .event-info { display: flex; flex-direction: column; gap: 0.5rem; margin-bottom: 1rem; }
        .event-info-item { display: flex; align-items: center; gap: 0.5rem; color: var(--cool-gray); font-size: 0.9rem; }
        .event-info-icon { font-size: 1.1rem; }
        ...
        .attendee-avatar { width: 32px; height: 32px; border-radius: 50%; background: linear-gradient(135deg, var(--cardinal-red), var(--cardinal-dark)); border: 2px solid white; margin-left: -0.5rem; display: flex; align-items: center; justify-content: center; color: white; font-size: 0.7rem; font-weight: 600; }
        .attendees-count { font-size: 0.85rem; color: var(--cool-gray); }
        ...
        .contact-icon { font-size: 2.25rem; margin-bottom: 0.75rem; }
        .contact-info h3 { font-size: 1.2rem; margin-bottom: 0.5rem; }
        ...
        .contact-map { border-radius: 15px; overflow: hidden; background: var(--sandstone); min-height: 380px; }
        .map-placeholder { display: flex; align-items: center; justify-content: center; height: 100%; min-height: 380px; background: var(--sandstone); color: var(--cool-gray); border-radius: 15px; padding: 1.5rem; font-size: 0.9rem; }
        .form-group { margin-bottom: 1rem; }
        .form-group label { display: block; margin-bottom: 0.5rem; font-weight: 600; color: var(--cool-gray); font-size: 0.9rem; }
        .form-group input, .form-group textarea { width: 100%; padding: 0.625rem; border: 2px solid var(--sandstone-dark); border-radius: 8px; font-family: 'Source Sans 3', sans-serif; font-size: 0.95rem; transition: border-color 0.3s; }
        ...
        @media (max-width: 768px) {
            ...
            .nav-links { position: fixed; top: 0; right: -100%; width: 280px; height: 100vh; background: white; flex-direction: column; padding: 6rem 2rem 2rem; box-shadow: -5px 0 20px rgba(0,0,0,0.1); transition: right 0.3s ease; align-items: flex-start; gap: 1.5rem; }
...
<body>
    <nav id="navbar">
        @if($logoUrl && file_exists(public_path('storage/' . $logoUrl)))
            <a href="/" class="logo">
                <img src="{{ asset('storage/' . $logoUrl) }}" alt="{{ $settings['site_name'] ?? 'RLMS' }}" style="max-height: 45px; max-width: 190px;">
            </a>
        @else
        ...
        <ul class="nav-links" id="navLinks">
            ...
            @auth
                <li><a href="{{ url('/dashboard') }}" class="btn-primary">Tableau de bord</a></li>
            @else
                <li><a href="{{ route('login') }}" class="btn-secondary">Connexion</a></li>
                @if (Route::has('register'))
                    <li><a href="{{ route('register') }}" class="btn-primary">Commencer</a></li>
                @endif
            @endauth
        </ul>
    </nav>
    ...
    <section class="hero">
        ...
        <div class="hero-content">
            @if($logoUrl && file_exists(public_path('storage/' . $logoUrl)))
                <img src="{{ asset('storage/' . $logoUrl) }}" alt="{{ $settings['site_name'] ?? 'RLMS' }}" style="max-height: 110px; margin-bottom: 1.5rem;">
            @endif
            <h1>Système de Gestion<br>de Laboratoire de Recherche</h1>
            <p>Optimisez vos opérations de recherche avec notre plateforme complète de gestion de laboratoire</p>
            <div class="hero-buttons">
                @auth
                    <a href="{{ url('/dashboard') }}" class="btn-primary">Aller au tableau de bord</a>
                @else
    ...
    <section class="features" id="features">
        ...
        <p class="section-subtitle">Tout ce dont vous avez besoin pour gérer efficacement votre laboratoire de recherche</p>

        <div class="features-grid">
            <div class="feature-card">
                <div class="feature-icon">🔬</div>
                <h3>Gestion des Équipements</h3>
                <p>Suivez et gérez tous vos équipements, matériaux et ressources de laboratoriume dans un système centralisé.</p>
            </div>
            <div class="feature-card">
                <div class="feature-icon">📅</div>
                <h3>Système de Réservation</h3>
                <p>Optimisez la réservation des équipements avec notre système intuitif de réservation et d'approbation.</p>
            </div>
            <div class="feature-card">
                <div class="feature-icon">📊</div>
                <h3>Suivi des Projets</h3>
                <p>Gérez les projets de recherche, collaborez avec les membres et suivez les avancées en temps réel.</p>
            </div>
            <div class="feature-card">
                <div class="feature-icon">🧪</div>
                <h3>Journal des Expériences</h3>
                <p>Documentez les expériences, joignez des fichiers et maintenez des dossiers de recherche complets.</p>
            </div>
            <div class="feature-card">
                <div class="feature-icon">📆</div>
                <h3>Gestion des Événements</h3>
                <p>Organisez événements, séminaires et réunions avec suivi RSVP et notifications.</p>
            </div>
            <div class="feature-card">
                <div class="feature-icon">⚙️</div>
                <h3>Suivi de Maintenance</h3>
                <p>Planifiez et suivez la maintenance des équipements pour des opérations optimales du laboratoire.</p>
            </div>
        </div>
    </section>
    ...
    <section class="about" id="about">
        ...
        <div class="about-content">
            <p>Notre laboratoire de recherche est dédié à l'avancement des connaissances scientifiques par la recherche innovante et la collaboration. Avec des équipements de pointe et une équipe de chercheurs dévoués, nous nous efforçons de contribuer significativement à notre domaine tout en formant la prochaine génération de scientifiques.</p>
            <p>La plateforme RLMS nous permet de gérer efficacement nos ressources, de coordonner les activités de recherche et de maintenir des dossiers complets de nos travaux scientifiques.</p>
    ...
    <section class="team" id="team">
        ...
        <p class="section-subtitle">Rencontrez les esprits brillants qui font avancer notre recherche</p>

        <div class="team-grid">
            <div class="team-card">
                <div class="team-avatar">RP</div>
                <h3 class="team-name">Chercheur Principal</h3>
                <p class="team-role">Directeur du Laboratoire</p>
                <p class="team-bio">Direction de recherches révolutionnaires en gestion de laboratoire et innovation scientifique.</p>
            </div>
            <div class="team-card">
                <div class="team-avatar">SC</div>
                <h3 class="team-name">Chercheur Senior</h3>
                <p class="team-role">Responsable Recherche</p>
                <p class="team-bio">Spécialisation en conception d'expériences et analyse de données pour projets complexes.</p>
            </div>
            <div class="team-card">
                <div class="team-avatar">EL</div>
                <h3 class="team-name">Scientifique de Laboratoire</h3>
                <p class="team-role">Spécialiste Équipements</p>
                <p class="team-bio">Gestion des équipements et garantie des conditions optimales de recherche.</p>
            </div>
            <div class="team-card">
                <div class="team-avatar">AR</div>
                <h3 class="team-name">Assistant de Recherche</h3>
                <p class="team-role">Équipe de Support</p>
                <p class="team-bio">Soutien aux activités de recherche et maintien des protocoles de laboratoire.</p>
            </div>
        </div>
    </section>
    ...
    <section class="research" id="research">
        ...
        <p class="section-subtitle">Explorez nos projets et publications actuels</p>
        ...
        <div class="research-grid">
            <div class="research-card">
                ...
                <div class="research-content">
                    ...
                    <p>Étude des mécanismes cellulaires et interactions moléculaires pour comprendre les processus biologiques fondamentaux.</p>
                    <p class="research-meta">15 Projets Actifs | 45 Publications</p>
                </div>
            </div>
            <div class="research-card">
                <div class="research-image">🔬</div>
                <div class="research-content">
                    <h3>Chimie Analytique</h3>
                    <p>Développement de méthodes analytiques avancées pour la caractérisation chimique et le contrôle qualité.</p>
                    <p class="research-meta">12 Projets Actifs | 38 Publications</p>
                </div>
            </div>
            <div class="research-card">
                <div class="research-image">🧪</div>
                <div class="research-content">
                    <h3>Science des Matériaux</h3>
                    <p>Exploration de nouveaux matériaux et leurs applications dans la technologie et l'industrie.</p>
                    <p class="research-meta">10 Projets Actifs | 32 Publications</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="events" id="events">
        ...
        <p class="section-subtitle">Rejoignez-nous pour séminaires, ateliers et présentations de recherche</p>
                ...
                <div class="event-content">
                    ...
                    <div class="event-info">
                        <div class="event-info-item">
                            ...
                            <span>{{ $event->event_time ?? 'Toute la journée' }}</span>
                        </div>
                        <div class="event-info-item">
                            <span class="event-info-icon">📍</span>
                            <span>{{ $event->location ?? 'À confirmer' }}</span>
                        </div>
                    </div>
                    ...
                    <div class="event-attendees">
                        ...
                        <span class="attendees-count">{{ $event->confirmedAttendees()->count() }} participants</span>
                    </div>
                    @auth
                        <a href="{{ route('events.show', $event) }}" class="btn-rsvp">Voir les détails</a>
                    @else
                        <a href="{{ route('login') }}" class="btn-rsvp">Réserver maintenant</a>
                    @endauth
                </div>
            </div>
            @endforeach
        ...
        @else
        <p style="text-align: center; color: var(--cool-gray);">Aucun événement à venir pour le moment. Revenez bientôt!</p>
        @endif
    </section>

    <section class="contact" id="contact">
        <h2 class="section-title">Contactez-Nous</h2>
        <p class="section-subtitle">Nous serions ravis d'avoir de vos nouvelles</p>
        ...
        <div class="contact-container">
            <div class="contact-grid">
                <div class="contact-info">
                    ...
                    <p>{!! nl2br(e($settings['location_address'] ?? 'Bâtiment de Recherche, Campus Universitaire, Ville, Pays')) !!}</p>
                </div>
                <div class="contact-info">
                    <div class="contact-icon">📧</div>
                ...
                <div class="contact-info">
                    ...
                    <p>{{ $settings['contact_phone'] ?? '+33 1 23 45 67 89' }}</p>
                    <p>{{ $settings['contact_hours'] ?? 'Lun-Ven, 9h-17h' }}</p>
                </div>
            </div>
            ...
            <div class="contact-bottom-grid">
                ...
                <div class="contact-map">
                    ...
                    @endphp
                    @if(!empty($mapSrc))
                        <iframe src="{{ $mapSrc }}" width="100%" height="100%" style="border:0; min-height: 380px;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                    @else
                        <div class="map-placeholder"><p>Aucune carte configurée. Ajoutez le code d'intégration dans les paramètres.</p></div>
                    @endif
                </div>
            </div>
        </div>
    ...
    <footer>
        <p>&copy; {{ date('Y') }} {{ config('app.name', 'RLMS') }}. Tous droits réservés.</p>
        <p style="margin-top: 0.5rem; font-size: 0.85rem;">Système de Gestion de Laboratoire de Recherche</p>
