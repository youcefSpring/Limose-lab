<x-app-layout>
    ...
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 lg:mb-8">
        <div>
            <h1 class="text-xl sm:text-2xl font-semibold">{{ __('messages.Notifications') }}</h1>
            <p class="text-zinc-500 dark:text-zinc-400 text-sm mt-1">{{ __('messages.View all your notifications') }}</p>
        </div>
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if(isset($notification->data['action_url']))
                                    <a href="{{ $notification->data['action_url'] }}" class="inline-flex items-center gap-1 text-sm text-accent-cyan hover:text-accent-cyan/80 font-medium mt-2">
                                        {{ $notification->data['action_text'] ?? __('messages.View Details') }}
                        ...
                        <div class="flex gap-2 flex-shrink-0">
                            @if(!$notification->read_at)
                                <form method="POST" action="{{ route('notifications.mark-read', $notification) }}">
                                    @csrf
                                    <button type="submit" class="p-2 rounded-xl glass hover:glass-card transition-all" title="{{ __('messages.Mark as read') }}">
                            ...
                            <form method="POST" action="{{ route('notifications.destroy', $notification) }}"
                                onsubmit="return confirm('{{ __('messages.Are you sure you want to delete this notification?') }}')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2 rounded-xl bg-accent-rose/10 text-accent-rose hover:bg-accent-rose/20 transition-all" title="{{ __('messages.Delete') }}">
    ...
    @else
        ...
        <div class="glass-card rounded-2xl p-12 text-center">
            ...
            <h3 class="text-xl font-semibold mb-2">{{ __('messages.No notifications') }}</h3>
            <p class="text-zinc-500 dark:text-zinc-400 max-w-md mx-auto">
                @if(request('filter') == 'unread')
                    {{ __('messages.You have no unread notifications.') }}
                @else
                    {{ __('messages.You\'re all caught up! No notifications to display.') }}
                @endif
            </p>
        </div>
    @endif
