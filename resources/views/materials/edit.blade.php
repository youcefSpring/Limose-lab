<x-app-layout>
    ...
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 lg:mb-8">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-semibold">{{ __('messages.Edit Material') }}</h1>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="glass-card rounded-2xl p-6">
                        <div class="mb-6 pb-4 border-b border-black/5 dark:border-white/5">
                            <h2 class="text-lg font-semibold mb-1">{{ __('messages.Material Information') }}</h2>
                            <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('messages.Update the material details') }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div class="md:col-span-2">
                                <label for="name" class="block text-sm font-medium mb-2">
                                    {{ __('messages.Material Name') }} <span class="text-accent-rose">*</span>
                                </label>
                                <input type="text" name="name" id="name" value="{{ old('name', $material->name) }}" required
                                    ...
                                    placeholder="{{ __('messages.e.g., Microscope Model X200') }}">
                                @error('name')
                            ...
                            <div>
                                ...
                                <select name="status" id="status" required
                                    ...
                                    <option value="available" {{ old('status', $material->status) == 'available' ? 'selected' : '' }}>{{ __('messages.Available') }}</option>
                                    <option value="maintenance" {{ old('status', $material->status) == 'maintenance' ? 'selected' : '' }}>{{ __('messages.Maintenance') }}</option>
                                    <option value="retired" {{ old('status', $material->status) == 'retired' ? 'selected' : '' }}>{{ __('messages.Retired') }}</option>
                                </select>
                                @error('status')
                                    <p class="mt-1.5 text-xs text-accent-rose">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="location" id="location" value="{{ old('location', $material->location) }}" required
                                    placeholder="{{ __('messages.e.g., Lab A - Shelf 3') }}"
                            ...
                            <div class="md:col-span-2">
                                <label for="maintenance_schedule" class="block text-sm font-medium mb-2">
                                    {{ __('messages.Maintenance Schedule') }}
                                </label>
                                <select name="maintenance_schedule" id="maintenance_schedule"
                                    ...
                                    <option value="">{{ __('messages.No scheduled maintenance') }}</option>
                                    <option value="weekly" {{ old('maintenance_schedule', $material->maintenance_schedule) == 'weekly' ? 'selected' : '' }}>{{ __('messages.Weekly') }}</option>
                                    <option value="monthly" {{ old('maintenance_schedule', $material->maintenance_schedule) == 'monthly' ? 'selected' : '' }}>{{ __('messages.Monthly') }}</option>
                                    <option value="quarterly" {{ old('maintenance_schedule', $material->maintenance_schedule) == 'quarterly' ? 'selected' : '' }}>{{ __('messages.Quarterly') }}</option>
                                    <option value="yearly" {{ old('maintenance_schedule', $material->maintenance_schedule) == 'yearly' ? 'selected' : '' }}>{{ __('messages.Yearly') }}</option>
                                </select>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="description" id="description" rows="5"
                                    ...
                                    placeholder="{{ __('messages.Brief description...') }}">{{ old('description', $material->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1.5 text-xs text-accent-rose">{{ $message }}</p>
                                @enderror
                            </div>
                ...
                <div class="space-y-6">
                    <div class="glass-card rounded-2xl p-6">
                        <div class="mb-6 pb-4 border-b border-black/5 dark:border-white/5">
                            <h2 class="text-lg font-semibold mb-1">{{ __('messages.Material Image') }}</h2>
                            <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('messages.Upload a clear image of the equipment') }}</p>
                        </div>
                    ...
                    <div class="glass-card rounded-2xl p-6">
                        <div class="flex flex-col gap-3">
                            <button type="submit" class="flex items-center justify-center gap-2 w-full bg-gradient-to-r from-accent-amber to-accent-coral px-6 py-3 rounded-xl font-semibold text-white hover:opacity-90 transition-all shadow-lg shadow-accent-amber/20">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                                </svg>
                                {{ __('messages.Update Material') }}
                            </button>
                            <a href="{{ route('materials.show', $material) }}" class="flex items-center justify-center px-5 py-3 rounded-xl glass hover:glass-card text-sm font-medium transition-all">
                                {{ __('messages.Cancel') }}
                            </a>
                        </div>
                    </div>
