    ...
    <nav class="flex-1 p-4 space-y-1 overflow-y-auto overflow-x-hidden" id="sidebar-nav">
        ...
        <div class="space-y-1">
            ...
            <div id="nav-personal" class="{{ app()->getLocale() === 'ar' ? 'mr-4 border-r-2 pr-2' : 'ml-4 border-l-2 pl-2' }} border-black/5 dark:border-white/5 space-y-1" style="display: {{ request()->routeIs('notifications.*', 'profile.*') ? 'block' : 'none' }}">
                ...
                <a href="{{ route('contact-messages.index') }}" class="nav-item {{ request()->routeIs('contact-messages.*') ? 'active' : '' }} flex items-center gap-3 px-3 py-2 rounded-lg {{ request()->routeIs('contact-messages.*') ? 'text-zinc-800 dark:text-white bg-black/5 dark:bg-white/5' : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-800 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5' }} transition-all">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                    </svg>
                    <span class="text-sm">{{ __('messages.Contact Messages') }}</span>
                </a>
