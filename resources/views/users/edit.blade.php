<x-app-layout>
    ...
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 lg:mb-8">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-semibold">{{ __('messages.Edit User') }}</h1>
    ...
    <div>
        <form method="POST" action="{{ route('users.update', $user) }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    ...
                    <div>
                        <label for="name" class="block text-sm font-medium mb-2">
                            {{ __('messages.Full Name') }} <span class="text-accent-rose">*</span>
                        </label>
                        <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" required
                            ...
                            placeholder="{{ __('messages.e.g., John Doe') }}">
                        @error('name')
                    ...
                    <div class="md:col-span-2 lg:col-span-2">
                        <label for="email" class="block text-sm font-medium mb-2">
                            {{ __('messages.Email Address') }} <span class="text-accent-rose">*</span>
                        </label>
                        <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" required
                            ...
                            placeholder="{{ __('messages.e.g., john@example.com') }}">
                        @error('email')
                            <p class="mt-2 text-sm text-accent-rose">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $user->phone) }}"
                            ...
                            placeholder="{{ __('messages.e.g., +1 234 567 8900') }}">
                        @error('phone')
                            <p class="mt-2 text-sm text-accent-rose">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="research_group" id="research_group" value="{{ old('research_group', $user->research_group) }}"
                            ...
                            placeholder="{{ __('messages.e.g., Biotechnology Lab') }}">
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="pending" {{ old('status', $user->status) == 'pending' ? 'selected' : '' }}>{{ __('messages.Pending') }}</option>
                            <option value="suspended" {{ old('status', $user->status) == 'suspended' ? 'selected' : '' }}>{{ __('messages.Suspended') }}</option>
                            <option value="banned" {{ old('status', $user->status) == 'banned' ? 'selected' : '' }}>{{ __('messages.Banned') }}</option>
                        </select>
                        @error('status')
                            <p class="mt-2 text-sm text-accent-rose">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2 lg:col-span-3">
                        ...
                        <textarea name="bio" id="bio" rows="3"
                            ...
                            placeholder="{{ __('messages.Brief description about the user...') }}">{{ old('bio', $user->bio) }}</textarea>
                        @error('bio')
                            <p class="mt-2 text-sm text-accent-rose">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <!-- Password (Optional) -->
            <div class="glass-card rounded-2xl p-5 lg:p-6">
                <h2 class="text-lg font-semibold mb-2">{{ __('messages.Change Password') }}</h2>
                <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-5">{{ __('messages.Leave blank to keep current password') }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    ...
                    <div>
                        ...
                        <input type="password" name="password" id="password"
                            ...
                            placeholder="{{ __('messages.Min 8 characters') }}">
                        @error('password')
                            <p class="mt-2 text-sm text-accent-rose">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="password" name="password_confirmation" id="password_confirmation"
                            ...
                            placeholder="{{ __('messages.Re-enter password') }}">
                    </div>
                </div>
            </div>
            ...
            <!-- Roles -->
            <div class="glass-card rounded-2xl p-5 lg:p-6">
                <h2 class="text-lg font-semibold mb-5">{{ __('messages.Roles') }} <span class="text-accent-rose">*</span></h2>
