<x-app-layout>
    ...
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 lg:mb-8">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-semibold">{{ __('messages.Add New User') }}</h1>
                <p class="text-zinc-500 dark:text-zinc-400 text-sm mt-1">{{ __('messages.Create a new user account') }}</p>
            </div>
        </div>
    </header>
    ...
    <div class="w-full">
        <form method="POST" action="{{ route('users.store') }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="suspended" {{ old('status') == 'suspended' ? 'selected' : '' }}>{{ __('messages.Suspended') }}</option>
                            <option value="banned" {{ old('status') == 'banned' ? 'selected' : '' }}>{{ __('messages.Banned') }}</option>
                        </select>
                        @error('status')
                            <p class="mt-2 text-sm text-accent-rose">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2 lg:col-span-3">
                        ...
                        <textarea name="bio" id="bio" rows="3"
                            ...
                            placeholder="{{ __('messages.Brief description about the user...') }}">{{ old('bio') }}</textarea>
                        @error('bio')
                            <p class="mt-2 text-sm text-accent-rose">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <!-- Password -->
            <div class="glass-card rounded-2xl p-5 lg:p-6">
                <h2 class="text-lg font-semibold mb-5">{{ __('messages.Password') }}</h2>

                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    <!-- Password -->
                    <div>
                        <label for="password" class="block text-sm font-medium mb-2">
                            {{ __('messages.Password') }} <span class="text-accent-rose">*</span>
                        </label>
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-sm font-medium mb-2">
                            {{ __('messages.Confirm Password') }} <span class="text-accent-rose">*</span>
                        </label>
