<x-app-layout>
    ...
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 lg:mb-8">
        <div>
            <h1 class="text-xl sm:text-2xl font-semibold">{{ __('messages.Events') }}</h1>
            <p class="text-zinc-500 dark:text-zinc-400 text-sm mt-1">{{ __('messages.Laboratory events and seminars') }}</p>
        </div>
    ...
    <x-modal name="filter-modal" :show="false" maxWidth="lg">
        <div class="p-6">
            <h2 class="text-xl font-semibold mb-6">{{ __('messages.Filter Events') }}</h2>
            <form method="GET" action="{{ route('events.index') }}" class="space-y-4">
                <div>
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="{{ __('messages.Search events...') }}"
                ...
                <div>
                    ...
                    <select name="type"
                        ...
                        <option value="seminar" {{ request('type') == 'seminar' ? 'selected' : '' }}>{{ __('messages.Seminar') }}</option>
                        <option value="workshop" {{ request('type') == 'workshop' ? 'selected' : '' }}>{{ __('messages.Workshop') }}</option>
                        <option value="conference" {{ request('type') == 'conference' ? 'selected' : '' }}>{{ __('messages.Conference') }}</option>
                        <option value="meeting" {{ request('type') == 'meeting' ? 'selected' : '' }}>{{ __('messages.Meeting') }}</option>
                    </select>
                </div>
                ...
                <table class="w-full">
                    <thead class="bg-zinc-50 dark:bg-surface-800/50 border-b border-black/5 dark:border-white/5">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-{{ app()->getLocale() === 'ar' ? 'right' : 'left' }} text-xs font-semibold uppercase tracking-wider">{{ __('messages.Date/Time') }}</th>
                            <th class="px-6 py-4 text-{{ app()->getLocale() === 'ar' ? 'right' : 'left' }} text-xs font-semibold uppercase tracking-wider">{{ __('messages.Location') }}</th>
                            <th class="px-6 py-4 text-center text-xs font-semibold uppercase tracking-wider">{{ __('messages.Attendees') }}</th>
                            ...
                            <tr class="hover:bg-zinc-50/50 dark:hover:bg-white/5 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        @endcan
                                        @can('delete', $event)
                                            <form method="POST" action="{{ route('events.destroy', $event) }}" class="inline-block" onsubmit="return confirm('{{ __('messages.Are you sure you want to delete this event?') }}')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
    ...
    @else
        ...
        <div class="glass-card rounded-2xl p-12 text-center">
            ...
            <h3 class="text-xl font-semibold mb-2">{{ __('messages.No events found') }}</h3>
            <p class="text-zinc-500 dark:text-zinc-400 mb-6 max-w-md mx-auto">
                @if(request()->hasAny(['search', 'type']))
                    {{ __('messages.No events match your current filters. Try adjusting your search criteria.') }}
                @else
                    {{ __('messages.Get started by creating a new event.') }}
                @endif
            </p>
            @if(request()->hasAny(['search', 'type']))
            ...
            @else
                @can('create', App\Models\Event::class)
                    <a href="{{ route('events.create') }}" class="inline-flex items-center gap-2 bg-gradient-to-r from-accent-amber to-accent-coral px-6 py-3 rounded-xl font-medium text-white hover:opacity-90 transition-opacity">
                        ...
                        {{ __('messages.Create First Event') }}
                    </a>
                @endcan
            @endif
        </div>
