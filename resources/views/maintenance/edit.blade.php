<x-app-layout>
    ...
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 lg:mb-8">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-semibold">{{ __('messages.Edit Maintenance Log') }}</h1>
                <p class="text-zinc-500 dark:text-zinc-400 text-sm mt-1">{{ __('messages.Update the maintenance log details') }}</p>
            </div>
        </div>
    </header>
        ...
        <form method="POST" action="{{ route('maintenance.update', $log) }}">
            ...
            <div class="glass-card rounded-2xl p-6">
                <div class="mb-6">
                    ...
                    <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('messages.Update the maintenance details') }}</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    ...
                    <div>
                        ...
                        <select name="maintenance_type" id="maintenance_type" required
                            ...
                            <option value="preventive" {{ old('maintenance_type', $log->maintenance_type) == 'preventive' ? 'selected' : '' }}>{{ __('messages.Preventive') }}</option>
                            <option value="corrective" {{ old('maintenance_type', $log->maintenance_type) == 'corrective' ? 'selected' : '' }}>{{ __('messages.Corrective') }}</option>
                            <option value="inspection" {{ old('maintenance_type', $log->maintenance_type) == 'inspection' ? 'selected' : '' }}>{{ __('messages.Inspection') }}</option>
                            <option value="calibration" {{ old('maintenance_type', $log->maintenance_type) == 'calibration' ? 'selected' : '' }}>{{ __('messages.Calibration') }}</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="scheduled" {{ old('status', $log->status) == 'scheduled' ? 'selected' : '' }}>{{ __('messages.Scheduled') }}</option>
                            <option value="in_progress" {{ old('status', $log->status) == 'in_progress' ? 'selected' : '' }}>{{ __('messages.In Progress') }}</option>
                            <option value="completed" {{ old('status', $log->status) == 'completed' ? 'selected' : '' }}>{{ __('messages.Completed') }}</option>
                            <option value="cancelled" {{ old('status', $log->status) == 'cancelled' ? 'selected' : '' }}>{{ __('messages.Cancelled') }}</option>
                        </select>
                        @error('status')
                            <p class="mt-1.5 text-xs text-accent-rose">{{ $message }}</p>
                        @enderror
            ...
            <div class="flex items-center justify-between gap-3 mt-6">
                <div>
                    @can('delete', $log)
                        <button type="button" onclick="if(confirm('{{ __('messages.Are you sure you want to delete this maintenance log?') }}')) { document.getElementById('delete-form').submit(); }"
                            class="px-5 py-2.5 rounded-xl text-accent-rose hover:bg-accent-rose/10 text-sm font-medium transition-all">
                            {{ __('messages.Delete Log') }}
                        </button>
                    @endcan
                </div>
                <div class="flex items-center gap-3">
