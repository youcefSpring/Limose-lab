<x-app-layout>
    ...
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 lg:mb-8">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-semibold">{{ __('maintenance.log_maintenance_activity') }}</h1>
                <p class="text-zinc-500 dark:text-zinc-400 text-sm mt-1">{{ __('maintenance.record_new_log') }}</p>
            </div>
        </div>
    </header>
        ...
        <form method="POST" action="{{ route('maintenance.store') }}">
            ...
            <div class="glass-card rounded-2xl p-6">
                <div class="mb-6">
                    ...
                    <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('maintenance.fill_maintenance_details') }}</p>
                </div>
...
<option value="preventive" {{ old('maintenance_type', 'preventive') == 'preventive' ? 'selected' : '' }}>{{ __('maintenance.preventive') }}</option>
                        <option value="corrective" {{ old('maintenance_type') == 'corrective' ? 'selected' : '' }}>{{ __('maintenance.corrective') }}</option>
                        <option value="inspection" {{ old('maintenance_type') == 'inspection' ? 'selected' : '' }}>{{ __('maintenance.inspection') }}</option>
                        <option value="calibration" {{ old('maintenance_type') == 'calibration' ? 'selected' : '' }}>{{ __('maintenance.calibration') }}</option>
                        </select>
...
<option value="scheduled" {{ old('status', 'scheduled') == 'scheduled' ? 'selected' : '' }}>{{ __('maintenance.scheduled') }}</option>
                        <option value="in_progress" {{ old('status') == 'in_progress' ? 'selected' : '' }}>{{ __('maintenance.in_progress') }}</option>
                        <option value="completed" {{ old('status') == 'completed' ? 'selected' : '' }}>{{ __('maintenance.completed') }}</option>
                        </select>
                        @error('status')
                            <p class="mt-1.5 text-xs text-accent-rose">{{ $message }}</p>
                        @enderror
