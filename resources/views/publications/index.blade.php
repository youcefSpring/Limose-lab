<x-app-layout>
    ...
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 lg:mb-8">
        <div>
            <h1 class="text-xl sm:text-2xl font-semibold">{{ __('messages.Publications') }}</h1>
            <p class="text-zinc-500 dark:text-zinc-400 text-sm mt-1">{{ __('messages.Manage research publications and papers') }}</p>
        </div>
    ...
    <x-modal name="filter-modal" :show="false" maxWidth="lg">
        <div class="p-6">
            <h2 class="text-xl font-semibold mb-6">{{ __('messages.Filter Publications') }}</h2>
            <form method="GET" action="{{ route('publications.index') }}" class="space-y-4">
                <div>
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="{{ __('messages.Search by title or authors...') }}"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="type"
                            ...
                            <option value="journal" {{ request('type') == 'journal' ? 'selected' : '' }}>{{ __('messages.Journal') }}</option>
                            <option value="conference" {{ request('type') == 'conference' ? 'selected' : '' }}>{{ __('messages.Conference') }}</option>
                            <option value="book" {{ request('type') == 'book' ? 'selected' : '' }}>{{ __('messages.Book') }}</option>
                            <option value="chapter" {{ request('type') == 'chapter' ? 'selected' : '' }}>{{ __('messages.Chapter') }}</option>
                            <option value="thesis" {{ request('type') == 'thesis' ? 'selected' : '' }}>{{ __('messages.Thesis') }}</option>
                            <option value="preprint" {{ request('type') == 'preprint' ? 'selected' : '' }}>{{ __('messages.Preprint') }}</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-2">{{ __('messages.Year') }}</label>
                        <select name="year"
                            class="block w-full {{ app()->getLocale() === 'ar' ? 'text-right' : '' }} px-4 py-2.5 bg-white dark:bg-surface-700/50 border border-black/10 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-accent-indigo/50 focus:border-accent-indigo transition-all">
                            <option value="">{{ __('messages.All Years') }}</option>
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        <option value="in_press" {{ request('status') == 'in_press' ? 'selected' : '' }}>{{ __('messages.In Press') }}</option>
                ...
                <table class="w-full">
                    <thead class="bg-zinc-50 dark:bg-surface-800/50 border-b border-black/5 dark:border-white/5">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-{{ app()->getLocale() === 'ar' ? 'right' : 'left' }} text-xs font-semibold uppercase tracking-wider">{{ __('messages.Authors') }}</th>
                            <th class="px-6 py-4 text-{{ app()->getLocale() === 'ar' ? 'right' : 'left' }} text-xs font-semibold uppercase tracking-wider">{{ __('messages.Year') }}</th>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        @can('delete', $publication)
                                            <form method="POST" action="{{ route('publications.destroy', $publication) }}" class="inline-block" onsubmit="return confirm('{{ __('messages.Are you sure you want to delete this publication?') }}')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
    ...
    @else
        ...
        <div class="glass-card rounded-2xl p-12 text-center">
            ...
            <h3 class="text-xl font-semibold mb-2">{{ __('messages.No publications found') }}</h3>
            <p class="text-zinc-500 dark:text-zinc-400 mb-6 max-w-md mx-auto">
                @if(request()->hasAny(['search', 'type', 'year', 'status']))
                    {{ __('messages.No publications match your current filters. Try adjusting your search criteria.') }}
                @else
                    {{ __('messages.Get started by adding your first publication.') }}
                @endif
            </p>
            @if(request()->hasAny(['search', 'type', 'year', 'status']))
            ...
            @else
                @can('create', App\Models\Publication::class)
                    <a href="{{ route('publications.create') }}" class="inline-flex items-center gap-2 bg-gradient-to-r from-accent-indigo to-accent-violet px-6 py-3 rounded-xl font-medium text-white hover:opacity-90 transition-opacity">
                        ...
                        {{ __('messages.Add First Publication') }}
                    </a>
                @endcan
            @endif
        </div>
