                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-wrap items-center gap-2">
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium
                                    {{ $project->status->value === 'active' ? 'bg-accent-emerald/10 text-accent-emerald' : '' }}
                                    {{ $project->status->value === 'completed' ? 'bg-accent-cyan/10 text-accent-cyan' : '' }}
                                    {{ $project->status->value === 'on_hold' ? 'bg-accent-amber/10 text-accent-amber' : '' }}">
                                    <span class="w-1.5 h-1.5 rounded-full bg-current"></span>
                                    {{ __(ucfirst(str_replace('_', ' ', $project->status->value))) }}
                                </span>
